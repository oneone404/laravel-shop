...
<div class="login-info-box text-center">
    ...
    <div class="member-rank" style="background: {{ $gradient }};">
        <strong>{{ $rank['name'] }}</strong>
        <img src="{{ asset($rank['image']) }}" alt="{{ $rank['name'] }}" style="height: 24px;">
    </div>
</div>
<div class="login-info-box text-center">
<style>
...
<div class="rank-progress-container">
    <div class="rank-icons-wrapper">
        ...
        <div class="rank-line-fill" style="width: {{ $progressLine }}%;"></div>
        <div class="rank-icons">
            @foreach ($ranks as $rank)
                <img src="{{ asset($rank['image']) }}" title="{{ $rank['name'] }}">
            @endforeach
        </div>
    </div>
    ...
    <div class="rank-bar">
        <div class="rank-bar-fill" style="width: {{ $progress }}%; background: linear-gradient(90deg, #ff9800, #ffb74d);"></div>
    </div>

    {{-- Thanh ngang hạng hiện tại -> hạng tiếp theo --}}
    <div class="rank-progress-bar">
        <div class="rank-step">
            <img src="{{ asset($currentRank['image']) }}" alt="{{ $currentRank['name'] }}">
        ...
        <div class="rank-step">
            <img src="{{ asset($nextRank['image']) }}" alt="{{ $nextRank['name'] }}">
    ...
    @if ($progress < 100)
        <div class="rank-missing">
            Còn Thiếu <strong>{{ number_format($nextRank['min'] - $totalDeposited) }} VND</strong>
        </div>
    @else
        <div class="rank-missing">
...
@if (trim($rank['name']) === 'Thành Viên Mới' || $codesPerMonth == 0)
...
@else
    ...
    @else
        <div class="mt-2 text-primary">
            Có <strong>{{ $codesPerMonth }}</strong> Mã Giảm Giá Chưa Nhận
        </div>
        <form action="{{ route('discount.claim') }}" method="POST" class="mt-2">
            @csrf
                ...
                <tbody>
                    @forelse ($codes as $index => $code)
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>
                                {{ substr($code->code, 0, 3) . '****' . substr($code->code, -3) }}
                                <button class="copy-btn" data-clipboard-text="{{ $code->code }}">
                                    <i class="fas fa-copy"></i>
                                </button>
                            </td>
                            <td>{{ number_format($code->discount_value) }} VND</td>
                            <td>{{ $expiryDate }}</td>
                        </tr>
                    @empty
                        <tr>
        ...
        <tr>
            <td>
                <span class="rank-cell">
                    {{ $rankItem['name'] }}
                    <img src="{{ asset('images/rank/' . $rankItem['image']) }}" alt="{{ $rankItem['name'] }}">
                </span>
            </td>
            <td><strong>{{ $rankItem['codes'] }}</strong></td>
        </tr>
    @endforeach
</tbody>
