...
<x-hero-header title="Mua Key VIP" description="" />
...
<div class="container">
    ...
    <div class="service__form mb-5">
        ...
        <form action="{{ route('gamekey.create') }}" method="POST" id="orderForm">
            @csrf
            ...
            <div class="service__form-group">
                ...
                <div class="select-wrapper" id="customSelect">
                    <div class="selected" data-value="com.vng.playtogether">
                        <img src="{{ asset('images/vng.png') }}" alt=""> PLAY TOGETHER VNG
                    ...
                    <div class="options">
                        ...
                        <div class="option" data-value="com.haegin.playtogether">
                            <img src="{{ asset('images/global.png') }}" alt=""> PLAY TOGETHER GLOBAL
                        </div>
                    </div>
            ...
            <div class="service__form-group">
                <label><i class="fas fa-hourglass-end"></i> HẠN SỬ DỤNG <img src="{{ asset('assets/img/icons/vip-pass.gif') }}" style="width:22px; height:22px; vertical-align:middle; margin-left:6px;" alt="VIP"></label>
            ...
            <div class="service__form-group">
                ...
                <input type="text" name="discount_code" id="discount_code" class="service__form-control" placeholder="Nhập Mã Nếu Có" value="{{ old('discount_code') }}">
            ...
            <div class="text-center">
                ...
                @else
                    <a href="{{ route('login') }}" class="service__btn service__btn--primary" style="min-width: 300px;">ĐĂNG NHẬP ĐỂ MUA</a>
                @endif
            </div>
        </form>
    </div>
        ...
        <div class="history-table-container">
            <table class="history-table">
                ...
                <tbody>
                    ...
                    <tr data-purchase-id="{{ $purchase->id }}" data-reset-count="{{ $purchase->reset_count ?? 0 }}">
                        <td>
                            <div class="game-cell">
                                @if($purchase->game === 'com.vng.playtogether')
                                    <img src="{{ asset('images/vng.png') }}" class="game-icon-img" title="VNG">
                                @elseif($purchase->game === 'com.haegin.playtogether')
                                    <img src="{{ asset('images/global.png') }}" class="game-icon-img" title="Global">
                                @else
                                    <span class="badge bg-secondary">{{ $purchase->game }}</span>
                                @endif
                                
                                <button class="btn-reset-device reset-trigger" 
                                        data-key="{{ $purchase->key_value }}" 
                        ...
                        <td>
                            <div class="key-cell">
                                <span>{{ substr($purchase->key_value, 0, 5) . '...' }}</span>
                                <button class="copy-btn" data-clipboard-text="{{ $purchase->key_value }}" title="Copy đầy đủ key">
                                    <i class="fas fa-copy"></i>
                                </button>
                            </div>
                        </td>
                        <td>
                            <div class="device-loading-container" data-key="{{ $purchase->key_value }}">
                        ...
                        <td>
                            <span class="badge bg-light text-dark">{{ $purchase->time_use }}</span>
                        </td>
                        <td>
                            <span class="price-badge">{{ number_format($purchase->price) }}</span>
                        </td>
                        <td>
                            <div class="time-badge">{{ $purchase->created_at->format('H:i:s - d/m/Y') }}</div>
                        </td>
                        <td>
                            <div class="expiry-loading-container" data-key="{{ $purchase->key_value }}">
                                <span class="device-badge">
                                    <i class="fas fa-spinner fa-spin"></i>
                                </span>
                            </div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const loadDeviceForKeys = () => {
        document.querySelectorAll('.device-loading-container').forEach(cont => {
            ...
            fetch('{{ route('user.ajax.get-key-details') }}', {
                method: 'POST',
                headers: {'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}'},
            ...
            openCustomModal(
                ...
                [
                    ...
                    {text: '<i class="fas fa-sync-alt"></i> Reset', class: 'modal-btn-danger', onclick: () => {
                        ...
                        fetch('{{ route('user.ajax.reset-devices') }}', {
                            method: 'POST',
                            headers: {'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}'},
        ...
        openCustomModal(
            ...
            [
                ...
                {text: '<i class="fas fa-trash"></i> Xóa ngay', class: 'modal-btn-danger', onclick: () => {
                    ...
                    fetch('{{ route('user.ajax.delete-device') }}', {
                        method: 'POST',
                        headers: {'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}'},
