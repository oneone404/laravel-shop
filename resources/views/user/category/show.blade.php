...
<section class="account-section">
    <div class="container no-padding">
        @if($category->type === 'random')
            {{-- Random Category - Multiple Random Group Cards --}}
            @php
            ...
            <div class="account-grid-new">
                @foreach($randomGroups as $group)
                    ...
                    <div class="account-item">
                        <a href="{{ $group->thumb ?? $category->thumbnail }}" class="account-image-wrapper sl-link">
                            <img src="{{ $group->thumb ?? $category->thumbnail }}" alt="{{ $category->name }}" class="account-image">
                            <div class="account-code" style="font-size: 10px; padding: 4px 10px;">RANDOM</div>
                        </a>
                        <div class="account-info-wrapper" style="padding: 15px;">
                            {{-- Số lượng đã bán và còn --}}
                            <div class="product-stats">
                                <span class="product-badge">ĐÃ BÁN <span class="badge-number-sold">{{ ($group->sold_count ?? 0) + 50 }}</span></span>
                                <span class="divider">|</span>
                                <span class="product-badge">CÒN LẠI <span class="badge-number-available">{{ $groupCount }}</span></span>
                            </div>
                                    ...
                                    @foreach($badges as $badge)
                                        @if(trim($badge))
                                            <div class="account-badge">
                                                <i class="fas {{ $icons[array_rand($icons)] }}"></i> {{ trim($badge) }}
                                            </div>
                                        @endif
                                    @endforeach
                                @else
                        ...
                        <div class="account-action-wrapper" style="padding: 15px;">
                            <div class="account-price" style="font-size: 18px;">
                                {{ number_format($group->price, 0, '.', '.') }} VND
                            </div>
                            @auth
                                <button class="account-button" style="font-size: 14px; padding: 10px 0;"
                                        ...
                                        data-category-badge="{{ $group->note }}"
                                        data-min-price="{{ $group->price }}"
                                        data-max-price="{{ $group->price }}"
                                        data-available-count="{{ $groupCount }}"
            ...
            @endif
        @else
            {{-- Normal Category - Individual Account Cards --}}
            <div class="account-grid-new">
                @forelse($accounts as $account)
                    <div class="account-item">
                        <a href="{{ $account->thumb }}?uid={{ $account->id }}"
                           class="account-image-wrapper sl-link"
                           onclick="increaseView({{ $account->id }})">
                            <img src="{{ $account->thumb }}" alt="Account Preview" class="account-image">
                            <div class="account-code">MS: {{ $account->id }}</div>
                        </a>
                        <div class="account-info-wrapper">
                            <div class="account-views">
                                <i class="fas fa-eye"></i>
                                <span class="view-count" id="view_{{ $account->id }}">
                                    {{ $account->views }}
                                </span>
                            </div>
                            <div class="account-badges">
                                @if($account->note)
                                    @foreach(explode("\n", trim($account->note)) as $noteLine)
                                        @if(trim($noteLine))
                                           @php
                                                $icons = ['fa-star', 'fa-check-circle', 'fa-fire', 'fa-bolt', 'fa-gem', 'fa-shield-alt'];
                                            ...
                                            <div class="account-badge">
                                                <i class="fas {{ $icon }}"></i> {{ trim($noteLine) }}
                                            </div>
                                        @endif
                                    @endforeach
                                @else
                                    ...
                                    <div class="account-badge">
                                        <i class="fas {{ $defaultIcons[array_rand($defaultIcons)] }}"></i> ZING ID
                                    </div>

                                    <div class="account-badge">
                                        <i class="fas {{ $defaultIcons[array_rand($defaultIcons)] }}"></i> TRẮNG THÔNG TIN
                                    </div>
                                @endif
                            </div>
                        </div>
                        <div class="account-action-wrapper">
                            <div class="account-price">{{ number_format($account->price, 0, '.', '.') }} VND</div>
                            @if($category->type === 'play')
                                <a href="{{ route('account.show', $account->id) }}" class="account-button">
                                    <i class="fas fa-info-circle"></i> XEM CHI TIẾT
                                </a>
                            @else
                                @auth
                                    <button class="account-button" data-id="{{ $account->id }}" data-price="{{ $account->price }}" onclick="openQuickPurchaseModal(this)">
    ...
    <div class="modal__content">
        ...
        <div class="modal__price-section">
            <div class="modal__price-row">
                ...
                <span class="modal__price-value">{{ $category->name}}</span>
            </div>
...
function openQuickPurchaseModal(btn) {
    ...
    if (purchaseBtn) {
        @auth
            purchaseBtn.innerHTML = '<i class="fas fa-wallet"></i> THANH TOÁN BẰNG SỐ DƯ ({{ number_format(auth()->user()->balance) }} VND)';
        @else
...
function submitAccountDirectPayment() {
    ...
    fetch('{{ route("direct-payment.create-account", ":id") }}'.replace(':id', currentAccountId), {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
...
function processRandomPurchase(quantity = 1) {
    ...
    fetch('{{ route("category.random-purchase", ":id") }}'.replace(':id', currentCategoryId), {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
...
function submitDirectPayment(type) {
    ...
    let url, body;
    if (type === 'random') {
        url = '{{ route("direct-payment.create-random", ":id") }}'.replace(':id', currentCategoryId);
        body = JSON.stringify({
            group_id: currentGroupId,
            quantity: quantity
        });
    } else {
        url = '{{ route("direct-payment.create-account", ":id") }}'.replace(':id', currentAccountId);
        body = JSON.stringify({});
    }
...
<div id="randomPurchaseModal" class="modal">
    <div class="modal__content">
        ...
        <div class="modal__body">
            {{-- Badge Info from Category Description --}}
