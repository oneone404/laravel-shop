  ...
  <div class="getkey-modal">
    ...
    <div class="getkey-body">
      <ul class="getkey-checklist">
        ...
        <li class="getkey-check-item">
          <i class="fas fa-check-circle"></i>
          <span>Sử Dụng Đến 23:59:59 - {{ date('d/m/Y') }}</span>
        </li>
      </ul>
  ...
  <div class="show-wrap">
    ...
    <div class="card">
      <div class="hack-head">
        ...
        @endif
        <div class="hack-title">{{ $hack->name }}<span class="status-dot {{ $hack->active ? 'active' : 'inactive' }}" title="{{ $hack->active ? 'Hoạt Động' : 'Bảo Trì' }}"></span></div>
      </div>
      ...
      {{-- Preview: Chỉ hiện ảnh đầu tiên --}}
      @if($imageCount > 0)
        <a href="{{ $gallery[0] }}" class="hack-gallery-preview sl-gallery">
          <img src="{{ $gallery[0] }}" alt="{{ $hack->name }}">
        </a>
        {{-- Hidden links cho các ảnh còn lại (để lightbox navigate được) --}}
        @foreach($gallery as $index => $img)
          @if($index > 0)
            <a href="{{ $img }}" class="hack-gallery-hidden sl-gallery"></a>
          @endif
        @endforeach
      @endif
...
<div id="wrap">
<a id="btnDownload"
   href="{{ route('download.hack', $hack) }}"
   class="btn-slide"
   data-active="{{ (int) ($hack->active ?? 0) }}"
   data-maint-msg="Bản Hack Đang Trong Quá Trình Bảo Trì Sửa Lỗi, Xin Cảm Ơn !"
   rel="nofollow">
  <span class="circle"><img src="{{ asset('images/vng.png') }}" alt="VNG" class="btn-icon-img"></span>
...
@if(!empty($hack->download_link_global))
  <a id="btnDownloadGlobal"
     href="{{ route('download.hack.global', $hack) }}"
     class="btn-slide4"
     data-active="{{ (int) ($hack->active ?? 0) }}"
     data-maint-msg="Bản Hack Đang Trong Quá Trình Bảo Trì Sửa Lỗi, Xin Cảm Ơn !"
     rel="nofollow">
    <span class="circle4"><img src="{{ asset('images/global.png') }}" alt="Global" class="btn-icon-img"></span>
    <span class="title4">Tải Global</span>
    <span class="title-hover4">Download</span>
  </a>
@endif

  <a href="{{ route('gamekey.form') }}" class="btn-slide3">
  ...
  <button type="button"
     id="btnKeyFree"
     class="btn-slide2"
     data-api-url="{{ route('hacks.free-key', $hack) }}"
     data-active="{{ (int) ($hack->active ?? 0) }}"
     data-maint-msg="Bản Hack Đang Trong Quá Trình Bảo Trì Sửa Lỗi, Xin Cảm Ơn !">
        ...
        @if(!empty($hack->description))
          ...
          @foreach($lines as $line)
            ...
            <div class="noti-item">
              ...
              <div class="noti-content">{!! $displayContent !!}</div>
            </div>
          @endforeach
        @else
    ...
    <aside class="card info-card-sidebar">
      ...
      <div class="info-wrap">
        <ul class="info-list">
            <li class="info-item"><span>Phiên Bản</span><span>{{ $hack->version ?? $hack->name }}</span></li>
            <li class="info-item"><span>Hệ Điều Hành</span><span>{{ $hack->platform ?? 'Android' }}</span></li>
            <li class="info-item"><span>Kích Thước</span><span>{{ $hack->size ?? '...' }}</span></li>
            <li class="info-item"><span>Nâng Cấp Lần Cuối</span><span>{{ optional($hack->updated_at)->format('d/m/Y H:i:s') ?? '—' }}</span></li>
        </ul>
      </div>
    </aside>
  </div>
