    ...
    public function purchasedAccounts(Request $request)
{
    ...
    // Lấy tài khoản game
    $gameAccounts = GameAccount::where('buyer_id', Auth::id())
        ->where('status', 'sold')
        ->get()
        ->each(function ($item) {
            $item->type = 'game';
    ...
    // Lấy tài khoản random từ hệ thống mới
    $randomAccounts = PurchasedRandomAccount::where('user_id', Auth::id())
        ->get()
        ->each(function ($item) {
            $item->type = 'random';
        });

    // Lấy tài khoản random từ hệ thống cũ (nếu muốn giữ lại lịch sử cũ)
    $oldRandomAccounts = RandomCategoryAccount::where('buyer_id', Auth::id())
        ->where('status', 'sold')
        ->get()
        ->each(function ($item) {
            $item->type = 'random';
    ...
    // Gộp & sắp xếp giảm dần theo thời gian mua
    $merged = $gameAccounts
        ->merge($randomAccounts)
        ->merge($oldRandomAccounts)
    ...
    public function getServiceDetail($id)
    {
        try {
            ...
            return response()->json([
                ...
                'server' => match ($service->server) {
                    'PTVNG' => 'PLAY TOGETHER VNG',
                    default => 'PLAY TOGETHER GLOBAL',
                },
                ...
                'admin_note' => $service->game_account ?? 'Không Có Tài Khoản'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'status' => 'error',
